...
class ControllerCustomerCustomer extends Controller {
	...
	public function phpexcel()
	{
		$this->load->language('customer/customer');
		$this->load->model('customer/customer');
			require "C:/photoolex_website/photoolex/admin/PHPExcel/PHPExcel.php";
		...
		{
			$value['birth'] = $value['yyyy'].'/'.$value['mm'].'/'.$value['dd'];
	...
	protected function getForm() {
		...
		if (isset($this->request->post['yyyy'])) {
			$data['yyyy'] = $this->request->post['yyyy'];
		} elseif (!empty($customer_info)) {
			$data['yyyy'] = $customer_info['yyyy'];
		} else {
			$data['yyyy'] = '';
		}
		if (isset($this->request->post['mm'])) {
			$data['mm'] = $this->request->post['mm'];
		} elseif (!empty($customer_info)) {
			$data['mm'] = $customer_info['mm'];
		} else {
			$data['mm'] = '';
		}
		if (isset($this->request->post['dd'])) {
			$data['dd'] = $this->request->post['dd'];
		} elseif (!empty($customer_info)) {
			$data['dd'] = $customer_info['dd'];
		} else {
			$data['dd'] = '';
		}	
		if (isset($this->request->post['country'])) {
		...
		$country_all = array('America','Canada','Japan','Britain','Germany','France','Italy','Australia','The United Arab Emirates');
